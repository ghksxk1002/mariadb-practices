-- inner join
-- 예제1 : employees 테이블과 타이틀 title 테이블을 join 하여 현제 직원의 이름과 직책을 모무 출력
select a.first_name, b.title
  from employees as a, titles as b
 where a.emp_no = b.emp_no    -- join 조건
   and b.to_date='9999-01-01'; -- row 선택 조건
   
-- 예제2 : employees 테이블과 title 테이블을 join 하여 직원의 이름과 직책을 출력하되 여성 엔지니어만 출력하세요.
...
-- 1) natural join
-- 두 테이블에 공통 컬럼이 있으면 별다른 조인 조건없이 암묵적으로 조인이 됨
-- 쓸일 없음
	  select a.first_name, b.title
        from employees as a 
...
-- 2) join ~ using
 select a.first_name, b.title
        from employees as a join titles as b using(emp_no)
where b.to_date='9999-01-01'; -- row 선택 조건
...
-- 실습문제 1
-- 현제 회사 상황을 반영한 직원별(group by 를 생각해볼수있고) 근무부서를 사번, 이름, 근무 부서로 출력해 보세요.
select a.emp_no '사번', a.first_name '이름', c.dept_name as '근무 부서'
...
-- 실습문제 2
-- 현제 회사에서 지급되고 있는 급여체게를 반영한 결과를 출력하세요.
-- 사번, 이름, 연본 형태로 출력하세요.
