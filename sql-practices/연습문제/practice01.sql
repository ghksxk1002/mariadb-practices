...
select emp_no, concat(first_name, ' ',last_name) as '전체 이름'
  from employees
 where emp_no = '10944';
...
-- 여직원과 남직원은 각 각 몇 명이나 있나요?
  select gender as '성별', count(gender) as '직원수'
    from employees
group by gender
...
select count(salary) as '총 직원수'
  from salaries;


-- 문제5.
-- 부서는 총 몇 개가 있나요?
select count(dept_no) as '총 부서수'
  from departments;
...
select count(to_date) as '현역 메니저수'
  from dept_manager 
...
-- 문제11
-- 사번이 13250(Zeydy)인 사원의 직책 변경 상황을 시간순으로 출력해보세요.
